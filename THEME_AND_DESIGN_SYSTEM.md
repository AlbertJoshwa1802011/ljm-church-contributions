...
- **`functions/api/appearance.js`** + **`member_preferences`** table (`migrations/0026_member_appearance.sql`, renumbered from 0011 to resolve a filename collision with `0011_events.sql`) store `{ email, accent_light, accent_dark }`, keyed by the **verified** Google email (writes reject unverified/legacy tokens). Identity is resolved with the shared `resolveViewer` in `_lib.js` (member self-service), not `requireAuth` (admin-only).
