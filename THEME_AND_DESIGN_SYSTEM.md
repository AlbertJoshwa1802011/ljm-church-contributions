...
| `--accent` / `--accent-deep` / `--accent-soft` | `#d97757` terracotta | `#e2825f` (desaturated) | Brand color, CTAs, active states |
...
The previous dark theme used `#000000`/`#0a0a0a` for backgrounds and a highly saturated `#ff6b4a` for the accent. Both were replaced:

- **Backgrounds** are now a warm near-black elevation scale (`#16140f` → `#1c1a14` → `#232019`), echoing the light theme's warm-paper tone instead of jumping to a cold, stark black. This is the same reasoning most platforms' dark-mode guidelines give for avoiding true `#000`: it reads as more premium and is easier on the eyes at night, while still giving cards a visible "step up" in elevation.
- **The accent color is desaturated**, not just lightened — `#e2825f` vs. the old `#ff6b4a` is the same hue family as the light theme's `#d97757`, just tuned to avoid looking like a neon warning color against a dark background. Contrast ratios were checked against WCAG AA (and mostly AAA) for text-sized use — softer does not mean lower-contrast here, just less saturated.
- **`--info` is a new token** (`#3f7a94` light / `#6fa8c2` dark, a muted teal) — added specifically so that removing every last purple/indigo accent (see below) didn't collapse multiple simultaneous chart/stat-card categories into indistinguishable colors. Where a 4th or 5th distinct hue was needed (e.g. the Overview stat-card row: goal/collected/remaining/count), `--info` fills that role instead of reintroducing purple.

## The purple/indigo purge

The old brand gradient (`#667eea` → `#764ba2`, plus several related indigo/violet/lavender shades) was left over from a pre-`theme.css` design pass and was still present in `style.css`, `script.js`, `impact.html`/`impact.js`, `members.html`, `index.html`, `admin.html`'s chart palette, and `admin-session.js`'s floating admin-mode bar. All of it was replaced with the current terracotta accent system (or, where a chart/legend needed a *distinct* second color rather than the primary brand color, `--info` or the existing semantic tokens).
