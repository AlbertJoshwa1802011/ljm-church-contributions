...
Before this feature, `members` was a flat list of individuals with no way to represent a household. Subscriptions (monthly membership dues) was tracked strictly per-member, with one global amount applied to every person individually.

The church's actual practice is: Subscriptions is a small monthly due paid **once per family**, normally by the head of the household — not once per person living in that household. Families and Subscriptions were built together because that's the requirement driving the data model.
...
- `functions/api/subscriptions.js`'s month view (`GET /api/subscriptions?month=YYYY-MM`) returns `paid`/`pending` scoped to **ungrouped individuals only**, plus a separate `families: { paid, pending }` list — a family's members are never double-counted as individuals once grouped.
...
- **Make head**: reassigns which member Subscriptions is billed to.
...
## Subscriptions (admin console → Giving → Subscriptions)
...
The Subscriptions tab shows two tables for the selected month:
...
The public `subscriptions.html` page mirrors this: family cards and individual cards both appear in the paid/pending grids, and a signed-in believer's personal status card (`sandhaPersonal`) automatically reflects their family's payment once they're grouped, by calling `GET /api/subscriptions?member=<name>` — which internally checks whether that member has a `family_id` and looks in the right table.

## API summary (`functions/api/subscriptions.js`, `functions/api/families.js`)
...
| `GET /api/subscriptions?month=YYYY-MM` | Month view: individuals + families, paid/pending, totals | public |
| `GET /api/subscriptions?member=Name` | One member's (or their family's) paid months for a year | public |
| `POST /api/subscriptions` | `action: mark_paid \| unmark`, with `familyId` or `memberId` | `manage_sandha` |
