/**
 * Enhanced Fund Handler - Supports Both Christmas Fund and Tech Fund
 * 
 * This is an enhanced version of your existing christmasfund.gs
 * It maintains backward compatibility while adding Tech Fund support
 * 
 * Usage:
 * - Christmas Fund: ?fund=christmas-fund (or ?fund=christmas)
...
function doGet(e) {
  ...
  // Detect format: Tech Fund has "date" and "amount" columns, Christmas Fund has month columns
  ...
  if (hasDateColumn && hasAmountColumn && hasMemberColumn) {
    ...
    // Christmas Fund Format: Member in column B, months as column headers (your existing logic)
  ...
  if(configSheet) {
    ...
    // If not found with specific key, try generic "goalamount"
    if (!found) {
      configData.forEach(row => {
        const key = row[0] ? row[0].toString().toLowerCase().trim() : '';
        if (key === "goalamount") {
...
/**
 * Process Christmas Fund format: Member in column B, months as column headers
