...
// Milestone v2 — the two-church model (Church of Light + City Worship Center).
//
//   GET    /api/churches           → public: active churches, sort_order asc
//          /api/churches?all=1     → admin (manage_funds): every church, any status
//   POST   /api/churches           → admin (manage_funds): create
//   PUT    /api/churches           → admin (manage_funds): update (body.id)
//   DELETE /api/churches?id=NN     → admin (manage_funds): archive (soft — status='archived')
//
// Churches are foundational org data (used to scope Events/Programs/Prayer/Contact),
// so mutation follows the same permission as funds (manage_funds) per
// docs/milestone-v2/05-backend-schema.md.
...
// Archive (soft delete) — churches are referenced by other tables' nullable
// church_id columns, so a hard delete would orphan history for no benefit.
