...
export async function onRequestGet(context) {
  ...
  try {
    ...
    // ── 18b2. Subscriptions: add believer → mark paid → visible → unmark → cleanup ──
    try {
      ...
      const mark = memberId ? await (await api(`/api/subscriptions`, {
        method: "POST",
        body: JSON.stringify({ action: "mark_paid", memberId, month, amount: 1, method: "cash" })
      })).json() : { success: false };

      const view = await (await fetch(`${origin}/api/subscriptions?month=${month}&_t=${ts}s`)).json();
      const inPaid = (view.paid || []).some(p => p.id === memberId);

      const unmark = memberId ? await (await api(`/api/subscriptions`, {
        method: "POST",
        body: JSON.stringify({ action: "unmark", memberId, month })
      })).json() : { success: false };

      record("subscriptions add-believer/mark/visible/unmark",
    ...
    } catch (e) { record("subscriptions add-believer/mark/visible/unmark", false, e.message); }

    // ── 18b3. Unauthenticated subscriptions write blocked ──
    try {
      const res = await fetch(`${origin}/api/subscriptions`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "mark_paid", memberId: 1, month: "2026-01" })
      });
      record("unauthenticated subscriptions write blocked", res.status === 401, `status=${res.status}`);
    } catch (e) { record("unauthenticated subscriptions write blocked", false, e.message); }
    ...
    // Subscriptions rows reference members by id — remove them BEFORE the members rows,
    ...
    try {
      await db.prepare(
        `UPDATE roles SET permissions = '["edit_purchases","edit_wishlist","manage_roles","view_members","manage_funds","delete_funds","view_audit","manage_expenses","manage_subscriptions"]' WHERE role_name = 'super_admin'`
      ).run();
    } catch (_) {}
  }
